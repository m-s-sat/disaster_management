var map = L.map('map').setView([20.5937, 82.9629], 5);
        ...
        async function loadcycloneData() {
            try {
                const response = await fetch('india_cyclone_data_2000_onwards.csv');
                ...
                rows.forEach(row => {
                    ...
                    cycloneEvents.push({ lat, lon, year, magnitude });
        ...
        document.querySelector(".search-btn").addEventListener("click",function(){
            ...
            const filteredEvents = cycloneEvents.filter(event => event.year.toString() === year);
            ...
            if (filteredEvents.length === 0) {
                resultsContainer.innerHTML = "<strong>No cyclone events found for this year</strong>";
                hideResultsAfterDelay();
                return;
            }
            
            let result = `<strong>cyclone Events in Year ${year}</strong><br><br>`;
        ...
        map.on('click', function(e) {
            ...
            let closestEvents = cycloneEvents.filter(event =>
            ...
            if (closestEvents.length > 0) {
                popupContent += '<br><strong>Recorded cyclones:</strong><br>';
                closestEvents.forEach(event => {
                    popupContent += `Year: ${event.year}, Magnitude: ${event.magnitude}<br>`;
                });
            } else {
                popupContent += `<br><strong>No cyclone event recorded here.</strong>`;
            }
            hideResultsAfterDelay();

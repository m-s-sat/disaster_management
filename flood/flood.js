var map = L.map('map').setView([20.5937, 82.9629], 5);
        ...
        async function loadfloodData() {
            try {
                const response = await fetch('flood_data_india.csv');
                ...
                rows.forEach(row => {
                    ...
                    floodEvents.push({ lat, lon, year, magnitude, loc});
        ...
        document.querySelector(".search-btn").addEventListener("click",function(){
            ...
            const filteredEvents = floodEvents.filter(event => event.year.toString() === year);
            ...
            if (filteredEvents.length === 0) {
                resultsContainer.innerHTML = "<strong>No flood events found for this year</strong>";
                hideResultsAfterDelay();
                return;
            }
            
            let result = `<strong>flood Events in Year ${year}</strong><br><br>`;
        ...
        map.on('click', function(e) {
            ...
            let closestEvents = floodEvents.filter(event =>
            ...
            if (closestEvents.length > 0) {
                popupContent += '<br><strong>Recorded floods:</strong><br>';
                closestEvents.forEach(event => {
                    popupContent += `Year: ${event.year}, Magnitude: ${event.magnitude}<br>`;
                });
            } else {
                popupContent += `<br><strong>No flood event recorded here.</strong>`;
            }
            hideResultsAfterDelay();
